...
/**
 * Preprocess Material for MkDocs admonition syntax.
 *
 *   !!!  type "Title"   non-collapsible callout (always shown)
 *   ???  type "Title"   collapsible, closed by default
 *   ???+ type "Title"   collapsible, open by default
 *       <4-space indented body>
 *
 * !!! emits <aside class="admonition admonition-<type>"> with a
 * .admonition-title heading. ??? / ???+ emit <details>.
 *
...
function preprocessAdmonitions(content: string): string {
    ...
    while (i < lines.length) {
        ...
        if (m) {
            ...
            const classes = type ? `admonition admonition-${type}` : 'admonition';
            if (isStatic) {
                out.push(`<aside class="${classes}">`);
                out.push(`<p class="admonition-title">${escapeHtml(summary)}</p>`);
                out.push('');
                out.push(...body);
                out.push('');
                out.push('</aside>');
            } else {
                out.push(`<details${isOpen ? ' open' : ''} class="${classes}">`);
...
/**
 * Allow the admonition HTML the preprocessor produces:
 *   <details ...>, </details>, <summary>, </summary>     ← ??? / ???+
 *   <aside class="admonition...">, </aside>              ← !!!
 *   <p class="admonition-title">...</p>                  ← !!! header
 *
 * Strips those tags; if nothing tag-like remains, the chunk is safe to
 * pass through. Other raw HTML still gets escaped.
 */
function isAdmonitionHtmlBlock(html: string): boolean {
    const stripped = html
        .replace(/<\/?(?:details|summary)(?:\s+open(?:="[^"]*")?)?(?:\s+class="[^"]*")?(?:\s+open(?:="[^"]*")?)?\s*>/gi, '')
        .replace(/<aside\s+class="admonition[^"]*"\s*>/gi, '')
        .replace(/<\/aside>/gi, '')
        .replace(/<p\s+class="admonition-title"\s*>([^<]*)<\/p>/gi, '$1');
...
export function parseMarkdown(content: string, opts: ParseOptions = {}): string {
    ...
    renderer.html = (html: string): string => {
        ...
        if (isAdmonitionHtmlBlock(trimmed)) {
            return html;
        }
